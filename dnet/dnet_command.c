/* dnet: commands; T11.261-T13.961; $DVS:time$ */
...
begin:
	...
	if (!cmd) {
		return 0;
#ifndef CHEATCOIN
	} else if (!g_dnet_xdag_mode && !strcmp(cmd, "close")) {
...
#ifndef CHEATCOIN
	} else if (!g_dnet_xdag_mode && (!strcmp(cmd, "exit") || !strcmp(cmd, "quit"))) {
        return -1;
#endif
...
#ifndef CHEATCOIN
		if (!g_dnet_xdag_mode) dnet_printf(out,
		...
		dnet_printf(out,
			...
			"  history [<N>] [<pattern>]     - print last N (20 by default) commands, select ones which contain pattern, if given\n"
		...
		if ((arg = strtok_r(0, " \t\r\n", &lasts))) {
			sscanf(arg, "%d", &N);
			if ((arg = strtok_r(0, " \t\r\n", &lasts))) sprintf(inbuf + DNET_COMMAND_MAX / 2, " | grep %s", arg);
		}
		sprintf(inbuf, "tail -n %u " HISTORY_FILE, N);
		if (arg) strcat(inbuf, inbuf + DNET_COMMAND_MAX / 2);
...
#ifndef CHEATCOIN
	} else if (!g_dnet_xdag_mode && !strcmp(cmd, "log")) {
        char *fname, *text;
        FILE *f;
	...
	} else if (!g_dnet_xdag_mode && !strcmp(cmd, "rename")) {
	...
	} else if (!g_dnet_xdag_mode && !strcmp(cmd, "streams")) {
		dnet_print_streams(out);
	} else if (!g_dnet_xdag_mode && (!strcmp(cmd, "tcp") || !strcmp(cmd, "udp"))) {
	...
	} else if (!g_dnet_xdag_mode && !strcmp(cmd, "untrust")) {
		char *name;
		int res;
	...
	} else if (!g_dnet_xdag_mode && (host = dnet_get_host_by_name(cmd))) {

/* dnet: threads; T11.231-T14.085; $DVS:time$ */
...
static void dnet_thread_work(struct dnet_thread *t) {
	...
	char *str, *port;
	uint32_t ipaddr;
	int res = 0, ip_to = 0, port_to = 0, crc_to = 0, proto = DNET_TCP, reuseaddr = 1, ipv6;
	...
	strcpy(buf, t->arg);
	port = strrchr(buf, ':');
	if (!port) { mess = "port is not given"; goto err; }
	*port++ = 0;
	str = buf;
	if (!*str) { mess = "host is not given"; goto err; }
	ipv6 = !!strchr(str, ':');
	if (ipv6) {
		paddr = (struct sockaddr *)&peeraddr6;
		peeraddr_len = sizeof(peeraddr6);
	} else {
		paddr = (struct sockaddr *)&peeraddr;
		peeraddr_len = sizeof(peeraddr);
	}
	// Fill in the address of server
	memset(paddr, 0, peeraddr_len);
	// Resolve port
	t->conn.port = atoi(port);

    // Create a socket
	t->conn.socket = socket((ipv6 ? AF_INET6 : AF_INET), (proto == DNET_TCP ? SOCK_STREAM : SOCK_DGRAM), (proto == DNET_TCP ? IPPROTO_TCP : IPPROTO_UDP));
	...
	if (ipv6) {
		...
		inet_pton(AF_INET6, str, &peeraddr6.sin6_addr);
		peeraddr6.sin6_port = htons(t->conn.port);
		memcpy(&ipaddr, peeraddr6.sin6_addr.s6_addr + 12, 4);
	} else {
		...
		ipaddr = peeraddr.sin_addr.s_addr;
	}
	t->conn.ipaddr = ntohl(ipaddr);
	...
	if (t->type == DNET_THREAD_SERVER || t->type == DNET_THREAD_FORWARD_FROM || proto == DNET_UDP) {
...
#endif
		res = bind(t->conn.socket, paddr, peeraddr_len);
        ...
        for(;;) {
            ...
            *t1 = *t;
            ...
            // no timeout limit...)
			begin:
			t1->conn.socket = accept(t->conn.socket, paddr, &peeraddr_len);
			...
			if (ipv6) {
				memcpy(&ipaddr, peeraddr6.sin6_addr.s6_addr + 12, 4);
				t1->conn.port = ntohs(peeraddr6.sin6_port);
			} else {
				ipaddr = peeraddr.sin_addr.s_addr;
				t1->conn.port = ntohs(peeraddr.sin_port);
			}
			if (g_n_inbound >= g_conn_limit || (dnet_connection_open_check && (*dnet_connection_open_check)(&t1->conn, ipaddr, t1->conn.port)))
		...
		res = connect(t->conn.socket, paddr, peeraddr_len);
